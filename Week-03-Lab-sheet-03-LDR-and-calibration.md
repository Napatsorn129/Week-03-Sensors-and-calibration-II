...
| -------- | ------------- | ---------------------- |
| มืดสนิท    | 200 kΩ – 2 MΩ | 0.016V – 0.157V        |
| ห้องปกติ   | 10 kΩ – 50 kΩ | 0.55V – 1.65V          |
| ใต้โคมไฟ  | 2 kΩ – 10 kΩ  | 1.65V – 2.75V          |
| กลางแจ้ง  | 200 Ω – 2 kΩ  | 2.75V – 3.235V         |
...
```c
แบบ Polynomial : y = 5E-05x2 - 0.1291x + 17.237R² = 0.9838
![alt text](<แบบ Polynomial.png>)
```
...
| -----------| ---- | ---------| -----------|
| ใต้โต๊ะ      | 45   | 0        |     23     |
| ห้องเรียน    | 2989 |  115     |   227      |
| หน้าต่าง     | 3612 | 251      |  387       |
...
```c
ไมโครคอนโทรลเลอร์ วัดได้แค่ "แรงดัน" ($V$) แต่ LDR เปลี่ยนค่าเป็น "ความต้านทาน" ($\Omega$. 
วงจรนี้เลยทำหน้าที่เปลี่ยนความต้านทานของ LDR ให้กลายเป็นแรงดันไฟฟ้า เพื่อให้ขา ADC อ่านค่าได้
```
2. ทำไมต้องเก็บข้อมูลหลายจุดก่อนทำ Calibration?
```c
กราฟ LDR ไม่ใช่เส้นตรง (มันโค้งแบบ Power Law). 
การวัดหลายๆ จุด (มืด-สว่างหลายระดับ) จะช่วยให้เราวาดเส้นโค้ง Trendline ได้ตรงความจริงที่สุด และช่วยถัวเฉลี่ยค่าที่คลาดเคลื่อนออกไป
```
3. ถ้า Noise มากกว่าสัญญาณจริง จะเกิดอะไรขึ้น?
```c
ค่า ADC และ Lux จะผันผวน ตัวเลขอ่านได้ไม่นิ่ง. 
ถ้าเอาไปสั่งงาน (เช่น เปิด-ปิดไฟอัตโนมัติ) หลอดไฟจะกะพริบติดๆ ดับๆ เพราะสัญญาณรบกวนแกว่งข้ามเกณฑ์ที่ตั้งไว้
```
4. ทำไมการวางตำแหน่งเซนเซอร์จึงสำคัญต่อ Signal Integrity?
```c
เพื่อป้องกันสัญญาณรบกวน 2 แบบ:  

1.แสงรบกวน: เงาบัง หรือแสงสะท้อนข้างเคียง ทำให้ Lux คลาดเคลื่อน. 

2.ไฟฟ้ารบกวน: สายไฟยาวไป หรืออยู่ใกล้ปลั๊ก/มอเตอร์ จะโดนคลื่นแม่เหล็กไฟฟ้าแทรกเข้ามาในสาย
```
5. ถ้า ADC มี Resolution ต่ำ จะส่งผลอย่างไรต่อความแม่นยำ?
```c
วัดแสงสังเกตความต่างได้ยาก. 
เช่น ADC 8-bit แบ่งสเต็ปได้แค่ 256 ขั้น แสงเปลี่ยนนิดหน่อยค่าจะไม่ขยับเลย ต่างจาก ADC 12-bit (4,096 ขั้น) ที่เก็บรายละเอียดการเปลี่ยนแปลงของแสงได้ถี่และแม่นยำกว่ามาก
```
...
| 2.2 | เลือกค่า R คงที่เหมาะสม (เช่น 10kΩ)     |         4            |        |
...
| 3.3 | จดข้อมูลเป็นตารางครบถ้วน              |         4            |        |
...
| 4.3 | อ่านสมการจากกราฟได้ถูกต้อง                       |          4           |        |
...
| ข้อ | รายการ                                   | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ----------------------------------------| -------------------- | ------ |
| 5.1 | ทดสอบจุดแสงใหม่ที่ไม่ได้ใช้ตอนเก็บข้อมูล          |           4          |        |
...
1. จุดไหนในใบงานที่คิดว่ายากที่สุด? การคุมแสงให้นิ่งตอนเก็บข้อมูล และการเลือก Trendline ใน Excel ให้ได้ค่า $R^2$ ใกล้ $1$ มากที่สุด
    
2. ถ้าทำใบงานนี้อีกครั้ง จะปรับปรุงอะไร? คุมแสงให้เสถียรกว่าเดิม และเขียนโค้ดหาค่าเฉลี่ย เพื่อลด Noise ของ ADC
    
3. ส่วนไหนที่คิดว่าตนเองเข้าใจดีมาก? การทำงานของวงจร Voltage Divider และการนำสมการจาก Excel ไปแปลงค่าในโค้ด
